...
class PathPlanner:
    ...
    def update_children(self, node_id):
        ...
        # ????
        self.nodes[node_id].cost = self.nodes[self.nodes[node_id].parent_id].cost + self.cost_to_come(self.nodes[node_id].traj_from_parent)

        if len(self.nodes[node_id].children_ids) == 0:
            return
    ...
    def rrt_star_planning(self):
        ...
        while not goal_reached: #Most likely need more iterations than this to complete the map!
            ...
            self.nodes[closest_node_id].children_ids.remove(len(self.nodes)-1) # remove new from closest node if rewired
            self.nodes[node_min_id].children_ids.append(len(self.nodes)-1)
            self.nodes[-1].parent_id = node_min_id
            self.nodes[-1].cost = cost_min
            self.nodes[-1].traj_from_parent = traj.copy()

            #Close node rewire
            # print("TO DO: Near point rewiring")
            for node_id in nn:
                if node_id == node_min_id:
                    continue
                ...
                if traj.size != 0:
                    ...
                    if self.cost_to_come(traj.copy()) + self.nodes[-1].cost < self.nodes[node_id].cost:
                        self.nodes[-1].children_ids.append(node_id) # new child is near
                        self.nodes[self.nodes[node_id].parent_id].children_ids.remove(node_id) # remove near from its parent children
                        self.nodes[node_id].parent_id = len(self.nodes) - 1 # near parent is new
                        self.nodes[node_id].cost = self.cost_to_come(traj.copy()) + self.nodes[-1].cost # update near cost
                        self.nodes[node_id].traj_from_parent = traj.copy() # update trajectory from new to near
                        self.update_children(node_id) # update near's children
    ...
    def recover_path(self, node_id = -1):
        ...
        while current_node_id > -1:
            ...
            self.window.add_point(self.nodes[current_node_id].point[0:2].copy().flatten(),radius=2,color=(0, 0, 255))
        path.reverse()
        return path
...
def main():
    ...
    path_planner.window.save_image('before.jpeg')
    ...
    path_planner.window.save_image('after.jpeg')
